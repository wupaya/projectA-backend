...
from .service import serviceDetails, serviceTask, serviceTag
from .serializer_1 import RegistrationSerializer, LoginInputSerializer, PublicPageSerializer, ServiceDetailsSerializer, ServiceTaskSerializer, ServiceTagSerializer
import pymongo
...
class service_details(APIView):
    def get(self, request):
        serializer = ServiceDetailsSerializer(serviceDetails())
        return Response (serializer.data, status.HTTP_200_OK)

class service_task(APIView):
    def get(self, request):
        serializer = ServiceTaskSerializer(serviceTask())
        return Response (serializer.data, status.HTTP_200_OK)

class service_tag(APIView):
    def get(self, request):
        serializer = ServiceTagSerializer(serviceTag())
        return Response (serializer.data, status.HTTP_200_OK)
