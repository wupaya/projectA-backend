...
class  Example(APIView):
    def get(sefl, request):
        #mongodb+srv://cluster0-t3qae.mongodb.net/test
        client = pymongo.MongoClient("mongodb+srv://mhsn06_test:1234@cluster0-t3qae.mongodb.net/test?retryWrites=true")
        db = client.test
        #client.find_one()
        #serializer = NoyonParameterInput(data=request.data)
        #if serializer.is_valid():
        nio = NoyonIO()
        nio.sum = nio.give_me_sum(request.query_params.get('num1'),request.query_params.get('num2'))
        #return Response(NoyonParameterOutput(nio).data, status=status.HTTP_201_CREATED)
        #return Response(serializer.errors,status=status.HTTP_400_BAD_REQUEST)
        return Response({"datafromdatabase": dumps(client.test.students_personal.find_one({"name":"AB"}))},status=status.HTTP_200_OK)
